...
export default function App() {
  ...
  return (
    <div className="flex flex-col h-full max-h-full bg-chatBg-light dark:bg-chatBg-dark text-slate-800 dark:text-slate-200 overflow-hidden border border-slate-200/50 dark:border-slate-800/50 rounded-2xl shadow-2xl glass">
      {/* Header */}
      <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-slate-200/50 dark:border-slate-850/50 bg-white/40 dark:bg-black/30 backdrop-blur-md">
        <div className="flex items-center gap-2.5">
          <div className="relative flex items-center justify-center w-8 h-8 rounded-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold shadow-md">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {messages.length === 0 ? (
          // Welcome Screen
          <div className="h-full flex flex-col justify-center items-center text-center space-y-5 px-4 animate-fade-in">
            <div className="w-16 h-16 rounded-2xl bg-slate-100 dark:bg-slate-900 flex items-center justify-center shadow-inner border border-slate-200/10">
            ...
            <div className="grid grid-cols-1 gap-2 w-full max-w-[320px]">
              {SUGGESTIONS.map((sug, idx) => (
                <button
                  ...
                  className="text-left text-xs px-3.5 py-2.5 rounded-xl border border-slate-200/60 dark:border-slate-800 bg-white/50 dark:bg-slate-900/30 hover:bg-slate-100 dark:hover:bg-slate-800/60 transition-all shadow-sm hover:scale-[1.01]"
                >
                  {sug}
                </button>
              ))}
          ...
          messages.map((msg, idx) => (
            ...
            >
              ...
              >
                <div className="prose dark:prose-invert text-xs md:text-sm leading-relaxed max-w-none break-words [&>ul]:list-disc [&>ul]:pl-4 [&>ol]:list-decimal [&>ol]:pl-4 [&>p]:mb-1.5 last:[&>p]:mb-0">
        ...
        {isLoading && (
          <div className="flex w-full justify-start animate-pulse">
            <div className="bg-slate-100 dark:bg-slate-900 rounded-2xl rounded-tl-none px-4 py-3 text-slate-800 dark:text-slate-200 border border-slate-200/20 flex items-center gap-1">
        ...
        {error && (
          <div className="flex w-full justify-center">
            <div className="flex items-center gap-2 bg-red-500/10 dark:bg-red-500/5 text-red-500 text-xs px-3.5 py-2.5 rounded-xl border border-red-500/20 max-w-[90%]">
      ...
      {/* Input area */}
      <div className="shrink-0 px-4 py-3 bg-white/40 dark:bg-black/30 backdrop-blur-md border-t border-slate-200/50 dark:border-slate-850/50">
